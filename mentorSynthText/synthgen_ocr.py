...
#edited in github function to check homography for mirror transformations############################################
...
def get_text_placement_mask(xyz,mask,plane,pad=2,viz=False):
    ...
    H = numpy.array([[1,0,0],[0,1,0],[0,0,1]]) # edited by forcing homography to 3x3 identity matrix

    Hinv,_ = cv2.findHomography(pts_fp_i32[0].astype('float32').copy(),
                                pts[0].astype('float32').copy(),
                                method=0)
    Hinv = numpy.array([[1,0,0],[0,1,0],[0,0,1]]) # edited by forcing homography to 3x3 identity matrix, Hinv of h is h
...
class RendererV3(object):
    ...
    def __init__(self, data_dir, max_time=None):
        ...
        self.min_char_height = 32 #8 #px
        self.min_asp_ratio = 0.4 #

        self.max_text_regions = 20 #7
    ...
    def filter_for_placement(self,xyz,seg,regions):
        ...
        for idx,l in enumerate(regions['label']):
            ...
            if res is not None:
                mask,H,Hinv = res
                #masks.append(mask) #edited out original code
                #Hs.append(H) #edited out original code
                #Hinvs.append(Hinv) #edited out original code
                #filt[idx] = True #edited out original code

                #edited in code to filter out mirroring
    ...
    def place_text(self,rgb,collision_mask,H,Hinv):
        ...
        # print (colorize(Color.RED,'original text: %s,\theight: %d'%(text, min_h), bold=True))
    ...
    def render_text(self,rgb,depth,seg,area,label,ninstance=1,viz=False):
        ...
        for i in range(ninstance):
            ...
            # reset the place_masks for every instance
            place_masks = []
            print (f"[*]Image size : {rgb.shape}")
            h, w = rgb.shape[:2]
            tmp = np.full((h, w), 0, dtype=np.uint8)
            # tmp = Image.open("C:\\Users\\angus\\mentorSynthText\\data\\bg_img\\79290432_p0_master1200.jpg") # edited to force place_mask to contain image
            place_masks.append(tmp)
            ...
            NUM_REP = self.max_text_regions #5 # re-use each region self.max_text_regions times:
            ...
            for idx in reg_range:
                ...
                if txt_render_res is not None:
                    ...
                    itext.append(text)
                    ibb.append(bb)
                    break # edited to generate only one character per image
            ...
            if placed:
                ...
                # print (colorize(Color.RED,'placed %dth instance'%(i), bold=True))
                # if viz:
