...
class RenderFont(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        self.min_nchar = 2
        self.min_font_h = 64 #px : 0.6*12 ~ 7px <= actual minimum height
    ...
    def sample_font_height_px(self,h_min,h_max):
        ...
        # rnd = h_max #edit to force rnd to be largest possible value
        h_range = h_max - h_min
    ...
    def render_sample(self,font,mask):
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            i += 1
            print (f"f_h{f_h}")
            font.size = f_h*5 # set the font-size
...
class TextSource(object):
    ...
    def sample_word(self,nline_max,nchar_max,niter=100):
        ...
        if not self.is_good([rand_word])[0] or len(rand_word)>nchar_max:
            # return [] #original
            print (f"[*]Returned char : {rand_word}")
            return 'a'
            # return rand_word #edited, force all words to be a single character that is always returned regardless of the condition
        else:
            return rand_word
