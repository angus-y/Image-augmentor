...
class Colorize(object):
    ...
    def color_text(self, text_arr, h, bg_arr):
        ...
        # fg_col, bg_col = ([255,0,0],[255,0,0]) #edited to force bg and fg to be a single color
        print (f'[*]Foreground color: {fg_col} Background color: {bg_col}') #edited in print
    ...
    def process(self, text_arr, bg_arr, min_h):
        ...
        # add border:
        # if 1: #edited to force border on all text
        if np.random.rand() < self.p_border: #edited by commenting out to force border on all text
            if min_h <= 15 : bsz = 1
